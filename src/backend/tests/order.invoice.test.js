...
jest.mock('../src/middleware/auth', () => {
  const authenticate = (req, res, next) => {
    req.user = authenticate.mockUser || { id: 1, role: 'user' };
    next();
  };
...
const createOrder = () => ({
  ...
  user: { id: 42, name: 'Asha Rao', email: 'asha@example.com' },
  items: [
    { id: 1, name: 'Ceramic Bowl', price: '40.00', qty: 1, lineTotal: '40.00' },
    { id: 2, name: 'Silver Pendant', price: '80.00', qty: 1, lineTotal: '80.00' },
  ],
});
...
describe('GET /api/orders/:id/invoice', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    authenticate.setUser({ id: 42, role: 'user' });
  });
  ...
  it('returns 403 for non-owner non-admin access', async () => {
    const mockOrder = createOrder();
    Order.findByPk.mockResolvedValue(mockOrder);
    authenticate.setUser({ id: 99, role: 'user' });
